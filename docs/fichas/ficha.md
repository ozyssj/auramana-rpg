    ...
    <script>
      ...
      `;
        ...
        function baixarFicha() {
            ...
            // salva iniciativa
            ficha.iniciativa = [];
            document.querySelectorAll('.iniciativa').forEach(div => {
                const nome = div.querySelector("label").innerText.trim();
                const atributo = div.querySelector(".atributo-iniciativa").value;
                const treinamento = div.querySelector(".treinamento-iniciativa").value;
                const bonus = div.querySelector(".bonus-iniciativa").value;
                const valor = div.querySelector(".valor-iniciativa").value;

                ficha.iniciativa.push({
                    nome,
                    atributo,
                    treinamento,
                    bonus,
            ...
            document.querySelectorAll('.defesa').forEach(div => {
                const nome = div.querySelector("label").innerText.trim();
                const atributo = div.querySelector(".atributo-defesa").value;
                const treinamento = div.querySelector(".treinamento-defesa").value;
                const bonus = div.querySelector(".bonus-defesa").value;
                const valor = div.querySelector(".valor-defesa").value;
        ...
        function carregarFicha() {
            ...
            reader.onload = function(e) {
                ...
                Object.keys(data).forEach(id => {
                    if (["pericias", "iniciativa", "defesas"].includes(id)) return;
                    const el = document.getElementById(id);
                    if (el) {
                        if (el.type === 'checkbox') {
                            el.checked = data[id];
                ...
                if (data.pericias) {
                    data.pericias.forEach(periciaData => {
                        const div = Array.from(document.querySelectorAll('.pericia'))
                            .find(d => d.querySelector("label").innerText.trim() === periciaData.nome);

                        if (div) {
                            div.querySelector(".atributo-pericia").value = periciaData.atributo;
                            div.querySelector(".treinamento-pericia").value = periciaData.treinamento;
                            div.querySelector(".bonus-pericia").value = periciaData.bonus;
                            div.querySelector(".valor-pericia").value = periciaData.valor;
                        }
                    });
                }
                ...
                // Carregar iniciativa
                if (data.iniciativa) {
                    data.iniciativa.forEach(initData => {
                        const div = Array.from(document.querySelectorAll('.iniciativa'))
                            .find(d => d.querySelector("label").innerText.trim() === initData.nome);

                        if (div) {
                            div.querySelector(".atributo-iniciativa").value = initData.atributo;
                            div.querySelector(".treinamento-iniciativa").value = initData.treinamento;
                            div.querySelector(".bonus-iniciativa").value = initData.bonus;
                            div.querySelector(".valor-iniciativa").value = initData.valor;
                        }
                    });
                }
                ...
                // Carregar defesas
                if (data.defesas) {
                    data.defesas.forEach(defData => {
                        const div = Array.from(document.querySelectorAll('.defesa'))
                            .find(d => d.querySelector("label").innerText.trim() === defData.nome);

                        if (div) {
                            div.querySelector(".atributo-defesa").value = defData.atributo;
                            div.querySelector(".treinamento-defesa").value = defData.treinamento;
                            div.querySelector(".bonus-defesa").value = defData.bonus;
                            div.querySelector(".valor-defesa").value = defData.valor;
                        }
                    });
                }

                alert("Ficha carregada com sucesso!");
                atualizarPericias(); // recalcula tudo
            };
